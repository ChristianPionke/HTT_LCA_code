...
def setup():
    ...
    for i in range(3):
        ...
        labels.append(tk.Label(root, font = ("Courier", 15), fg = colors[i], text = '%s:\n%.2f %s' % (varnams[i],0.0, varnams[tempvar+3]))) #Making the Tkinter labels to store numerical temperatures
...
def updateWindow():
    ...
    #For the three temperature values: log the yvalue, print the value to excel, update the label, and add the plot to be drawn later
    for i in range(3):
        yvals[i].append(Eqs(0, tempdata[i])) #Put i's data into i's array
        worksheet.write(chr(i+66)+str(sheetrow+1), Eqs(0, tempdata[i]))
        labels[i].config(text = '%s:\n%.2f %s' % (varnams[i], Eqs(0, tempdata[i]), varnams[tempvar+3]))
    ...
    end = time.time() #Log the stop time
...
def Eqs(eqselect, x):
    ...
    if(eqselect == 0):                      #Eqs of 0 is LCA mapping
...
def Checkover(internal):
    internal = Eqs(0, internal)             #Turn raw data into temperature
...
root.title("LCA")                               #Setting the title of the window
...
varnams = ['Temp1', 'Temp2', 'Internal', 'C', 'F']  #The names of the variables to print
...
colors = ['red', 'darkgreen', 'blue']               #The colors on the graph
period = 1/freqvar
