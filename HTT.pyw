...
class Menu:
    def __init__(self):
        ...
        self.menuloop = tk.Tk()
        self.menuloop.title("Temperature")
...
#Changes from the menu to the display
def setup():
...
#Updates the window and writes to the Excel file
def updateWindow():
    ...
    for i in range(3):
        ...
        labels[i].config(text = '%s:\n%.2f %s' % (varnams[i], avgvals[i], varnams[tempvar+3]))
...
#Function to map interpretation equations to the inputs
def Eqs(eqselect, x):
...
root.title("HTT")                               #Setting the title of the window
...
varnams = ['Max.', 'Avg.', 'Min.', 'C', 'F']    #The names of the variables to print
...
#Initializing the Excel library
